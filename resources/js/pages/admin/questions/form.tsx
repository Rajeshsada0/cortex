...
interface QuestionData {
    ...
    is_active?: boolean;
    options?: Array<{ option_key: 'A' | 'B' | 'C' | 'D'; option_text: string; rationale?: string }>;
...
export default function QuestionForm({
    ...
    // Form state
    const [code, setCode] = useState(question?.code || `Q-${Math.random().toString(36).substring(2, 7).toUpperCase()}`);
    const [subjectId, setSubjectId] = useState<number>(question?.subject_id || (subjects[0]?.id ?? 1));
    ...
    const [subtopicId, setSubtopicId] = useState<number | ''>(question?.subtopic_id || '');
    ...
    const [imageCaption, setImageCaption] = useState(question?.image_caption || '');
    ...
    const initialOptions = question?.options?.length === 4
        ? question.options
        : defaultOptions;

    const [options, setOptions] = useState(initialOptions);
    const [correctOption, setCorrectOption] = useState<'A' | 'B' | 'C' | 'D'>(question?.correct_option || 'A');

    // 3-Tier Clinical Breakdown
    const [foundationExplanation, setFoundationExplanation] = useState(question?.foundation_explanation || '');
    const [integrationExplanation, setIntegrationExplanation] = useState(question?.integration_explanation || '');
    const [applicationExplanation, setApplicationExplanation] = useState(question?.application_explanation || '');

    // Recall Anchors
    const [learningObjective, setLearningObjective] = useState(question?.learning_objective || '');
    ...
    // Status
    const [isActive, setIsActive] = useState<boolean>(question?.is_active ?? true);
    ...
    const handleOptionTextChange = (key: 'A' | 'B' | 'C' | 'D', text: string) => {
        setOptions((prev) =>
            prev.map((opt) => (opt.option_key === key ? { ...opt, option_text: text } : opt))
        );
    };

    const handleOptionRationaleChange = (key: 'A' | 'B' | 'C' | 'D', rationale: string) => {
        setOptions((prev) =>
            prev.map((opt) => (opt.option_key === key ? { ...opt, rationale } : opt))
        );
    };
    ...
    return (
        <>
            <Head title={`${isEdit ? 'Edit' : 'Author'} Clinical Vignette MCQ — Cortex Admin`} />

            <form onSubmit={handleSubmit} className="flex flex-col gap-6 p-4 sm:p-6 lg:p-8 w-full">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border pb-6">
                        ...
                        <div>
                            <div className="flex items-center gap-2">
                                <h1 className="text-xl sm:text-2xl font-black text-foreground">
                                    {isEdit ? `Edit Clinical Vignette: ${question?.code}` : 'Author New Clinical Vignette MCQ'}
                                </h1>
                                <Badge variant={isActive ? 'default' : 'secondary'} className="text-[10px] font-bold">
                                    {isActive ? 'Published Active' : 'Draft'}
                                </Badge>
                            </div>
                            <p className="text-xs text-muted-foreground mt-0.5">
                                High-Yield Postgraduate Medical Vignette Authoring Suite
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2.5">
                        ...
                        <Button
                            type="submit"
                            disabled={processing}
                            className="bg-[#0066FF] hover:bg-[#0052cc] text-white text-xs font-bold gap-1.5 shadow-md px-5"
                        >
                ...
                <div className="rounded-2xl border border-border bg-card p-5 sm:p-6 shadow-xs space-y-4">
                    <div className="flex items-center gap-2 border-b border-border/60 pb-3">
                        ...
                        <h2 className="text-sm font-bold text-foreground">1. Curriculum Taxonomy &amp; Exam Relevance</h2>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        <div className="space-y-1.5">
                            <Label htmlFor="q-code" className="text-xs font-bold">Question Code</Label>
                            <Input
                                id="q-code"
                                value={code}
                        ...
                        <div className="space-y-1.5">
                            <Label htmlFor="q-subject" className="text-xs font-bold">Subject</Label>
                            <select
                                id="q-subject"
                                value={subjectId}
                        ...
                        <div className="space-y-1.5">
                            <Label htmlFor="q-topic" className="text-xs font-bold">Topic</Label>
                            <select
                                id="q-topic"
                                value={topicId}
                        ...
                        <div className="space-y-1.5">
                            <Label htmlFor="q-subtopic" className="text-xs font-bold">Subtopic (Optional)</Label>
                            <select
                                id="q-subtopic"
                                value={subtopicId}
                                onChange={(e) => setSubtopicId(e.target.value ? parseInt(e.target.value) : '')}
                                className="w-full rounded-xl border border-border bg-background px-3 py-2 text-xs font-semibold text-foreground focus:border-[#0066FF] focus:outline-none"
                            >
                    ...
                    <div className="space-y-1.5 pt-2 border-t border-border/60">
                        <Label className="text-xs font-bold">Relevant Exam Pathways</Label>
                                ...
                                return (
                                    <button
                                        ...
                                        className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-xs font-bold transition-all ${
                                            selected
                                                ? 'border-[#0066FF] bg-blue-50 dark:bg-blue-950/60 text-[#0066FF]'
                                                : 'border-border bg-card text-muted-foreground hover:bg-muted/40'
                                        }`}
                                    >
                                        <span className={`size-3 rounded-full flex items-center justify-center ${selected ? 'bg-[#0066FF] text-white' : 'border border-neutral-400'}`}>
                                            {selected && <Check className="size-2" />}
                                        </span>
                                        <span>{ep.label}</span>
                                    </button>
                                );
                ...
                <div className="rounded-2xl border border-border bg-card p-5 sm:p-6 shadow-xs space-y-4">
                    <div className="flex items-center gap-2 border-b border-border/60 pb-3">
                        ...
                        <h2 className="text-sm font-bold text-foreground">2. Clinical Vignette Stem &amp; Diagnostic Imaging</h2>
                    </div>
                    ...
                    <div className="space-y-1.5">
                        ...
                        <textarea
                            ...
                            className="w-full rounded-xl border border-border bg-background p-3 text-xs sm:text-sm font-normal text-foreground leading-relaxed focus:border-[#0066FF] focus:outline-none"
                        />
                    </div>
                ...
                <div className="rounded-2xl border border-border bg-card p-5 sm:p-6 shadow-xs space-y-4">
                    <div className="flex items-center justify-between border-b border-border/60 pb-3">
                        <div className="flex items-center gap-2">
                            ...
                            <h2 className="text-sm font-bold text-foreground">3. Multiple Choice Options &amp; Differential Rationales</h2>
                        </div>
                        <div className="text-xs text-muted-foreground font-medium">
                            Select the radio button for the <span className="font-bold text-emerald-600 dark:text-emerald-400">Correct Option</span>
                        </div>
                    </div>
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-start gap-3">
                                        <button
                                            type="button"
                                            onClick={() => setCorrectOption(opt.option_key)}
                                        ...
                                        <div className="flex-1 space-y-2">
                                            ...
                                            <div className="space-y-1 pt-1">
                                                <Label className="text-[11px] font-semibold text-muted-foreground">
                                                    Rationale for Option {opt.option_key} (Why right / Why wrong)
                                                </Label>
                                                <textarea
                                                    value={opt.rationale || ''}
                                                    onChange={(e) => handleOptionRationaleChange(opt.option_key, e.target.value)}
                                                    rows={2}
                                                    placeholder={`Explain why Choice ${opt.option_key} is ${isCorrect ? 'the accurate diagnostic/therapeutic choice' : 'incorrect' }...`}
                                                    className="w-full rounded-xl border border-border bg-background p-2.5 text-xs text-foreground focus:border-[#0066FF] focus:outline-none"
                                                />
                                            </div>
                                        </div>
                                    </div>
                ...
                <div className="rounded-2xl border border-border bg-card p-5 sm:p-6 shadow-xs space-y-4">
                    <div className="flex items-center gap-2 border-b border-border/60 pb-3">
                        ...
                        <h2 className="text-sm font-bold text-foreground">4. 3-Tier Postgraduate Clinical Explanation</h2>
                    </div>

                    <div className="space-y-3">
                        <div className="space-y-1.5">
                            <div className="flex items-center justify-between">
                                <Label htmlFor="q-foundation" className="text-xs font-bold text-foreground flex items-center gap-1.5">
                                    <span className="flex size-5 items-center justify-center rounded-full bg-blue-100 text-blue-800 dark:bg-blue-950 dark:text-blue-300 text-[10px] font-black">1</span>
                                    <span>Layer 1: Foundation (Core Basic Science / Pathophysiology)</span>
                                </Label>
                            </div>
                            <textarea
                                id="q-foundation"
                                value={foundationExplanation}
                                onChange={(e) => setFoundationExplanation(e.target.value)}
                                rows={3}
                                required
                                placeholder="Explain the underlying anatomical, biochemical, pharmacological, or physiological mechanisms..."
                                className="w-full rounded-xl border border-border bg-background p-3 text-xs text-foreground leading-relaxed focus:border-[#0066FF] focus:outline-none"
                            />
                        </div>

                        <div className="space-y-1.5">
                            <div className="flex items-center justify-between">
                                <Label htmlFor="q-integration" className="text-xs font-bold text-foreground flex items-center gap-1.5">
                                    <span className="flex size-5 items-center justify-center rounded-full bg-indigo-100 text-indigo-800 dark:bg-indigo-950 dark:text-indigo-300 text-[10px] font-black">2</span>
                                    <span>Layer 2: Integration (Clinical Reasoning &amp; Differential Diagnosis)</span>
                                </Label>
                            </div>
                            <textarea
                                id="q-integration"
                        ...
                        <div className="space-y-1.5">
                            <div className="flex items-center justify-between">
                                <Label htmlFor="q-application" className="text-xs font-bold text-foreground flex items-center gap-1.5">
                                    <span className="flex size-5 items-center justify-center rounded-full bg-purple-100 text-purple-800 dark:bg-purple-950 dark:text-purple-300 text-[10px] font-black">3</span>
                                    <span>Layer 3: Application (Therapeutic Guidelines &amp; Clinical Next Steps)</span>
                                </Label>
                            </div>
                            <textarea
                                id="q-application"
                                value={applicationExplanation}
                                onChange={(e) => setApplicationExplanation(e.target.value)}
                                rows={3}
                                required
                ...
                <div className="rounded-2xl border border-border bg-card p-5 sm:p-6 shadow-xs space-y-4">
                    <div className="flex items-center gap-2 border-b border-border/60 pb-3">
                        ...
                        <h2 className="text-sm font-bold text-foreground">5. High-Yield Learning Objectives &amp; Memory Pegs</h2>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                            <Label htmlFor="q-lo" className="text-xs font-bold">Key Learning Objective</Label>
                            <textarea
                                id="q-lo"
                                ...
                                className="w-full rounded-xl border border-border bg-background p-3 text-xs text-foreground focus:border-[#0066FF] focus:outline-none"
                            />
                        </div>

                        <div className="space-y-1.5">
                            <Label htmlFor="q-peg" className="text-xs font-bold">High-Yield Memory Peg / Mnemonic (Optional)</Label>
                            <textarea
                                id="q-peg"
                                value={memoryPeg}
                ...
                <div className="flex items-center justify-between border-t border-border pt-4">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <Button
                            type="submit"
                            disabled={processing}
                            className="bg-[#0066FF] hover:bg-[#0052cc] text-white text-xs font-bold gap-1.5 shadow-md px-6"
                        >
                            <Save className="size-3.5" />
                            {isEdit ? 'Save Changes' : 'Publish Question'}
                        </Button>
                ...
                <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
                    <DialogHeader>
                        <div className="flex items-center justify-between border-b border-border pb-3">
                            <div className="flex items-center gap-2">
                                <Badge variant="outline" className="font-mono text-xs text-sky-600">{code || 'Q-SAMPLE'}</Badge>
                                <DialogTitle className="text-sm font-bold">
                                    {selectedSubject?.name} · {selectedTopic?.name}
                                </DialogTitle>
                            </div>
                            <Badge className="bg-blue-600 text-white text-[10px] font-bold">{difficulty}</Badge>
                        </div>
                    </DialogHeader>
                    ...
                    <div className="space-y-4 py-2">
                        ...
                        <div className="space-y-2">
                            <div className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Options</div>
                            {options.map((opt) => (
                                <div
                                    key={opt.option_key}
                                ...
                                >
                                    ...
                                    <span className="pt-0.5">{opt.option_text || `Option ${opt.option_key} content...`}</span>
                                </div>
                            ))}
                        </div>
                        ...
                        {learningObjective && (
                            <div className="rounded-xl border border-sky-300 dark:border-sky-800 bg-sky-50 dark:bg-sky-950/40 p-3 text-xs text-sky-900 dark:text-sky-200">
